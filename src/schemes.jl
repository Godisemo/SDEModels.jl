...
function step(model::AbstractSDE, scheme::EulerMaruyama, current_state::AbstractState, Δw)
  μ = drift(model, current_state)
  σ = diffusion(model, current_state)
  statevalue(current_state) + μ * scheme.Δt + σ * Δw
end

function step{M}(model::AbstractSDE{1,M}, scheme::Milstein, current_state::AbstractState, Δw)
  μ = drift(model, current_state)
